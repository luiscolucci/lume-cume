...
import {
  ...
  Filter,
  Coins,
} from "lucide-react"; // Adicionei Coins
...
export default function Reports() {
  ...
  // Novo Estado para Somar os Trocos Retidos (Lucro)
  const [totalTrocoLucro, setTotalTrocoLucro] = useState(0);
  ...
  async function handleFilter() {
    ...
    try {
      ...
      let somaTotal = 0;
      let somaItens = 0;
      let somaTrocoLucro = 0; // Variável para somar os trocos
      ...
      querySnapshot.forEach((doc) => {
        ...
        if (dataVenda >= start && dataVenda <= end) {
          if (
            selectedSeller !== "" &&
            rawData.vendedor_uid !== selectedSeller
          ) {
            return;
          }
          ...
          // LÓGICA PARA EXTRAIR O VALOR DO TROCO DA OBSERVAÇÃO
          // A string salva é: "Troco de R$ 5.00 retido como lucro"
          let valorTrocoNestaVenda = 0;
          if (rawData.obs && rawData.obs.includes("retido como lucro")) {
            // Tenta extrair o número usando Regex
            const match = rawData.obs.match(/R\$\s?([0-9.]+)/);
            if (match) {
              valorTrocoNestaVenda = parseFloat(match[1]);
            }
          }
          ...
          listaVendas.push({
            ...
            obsDisplay: rawData.obs || "-", // Para mostrar na tabela
            isTrocoLucro: valorTrocoNestaVenda > 0, // Flag para pintar a linha
          });

          somaTotal += Number(rawData.total) || 0;
          somaTrocoLucro += valorTrocoNestaVenda;

          if (rawData.itens) {
            somaItens += rawData.itens.reduce(
      ...
      setTotalTrocoLucro(somaTrocoLucro);
  ...
  function exportPDF() {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text("Relatório de Vendas - Lume Cume", 14, 15);

    doc.setFontSize(10);
    doc.text(
      `Período: ${new Date(startDate + "T00:00:00").toLocaleDateString(
        "pt-BR"
      )} a ${new Date(endDate + "T00:00:00").toLocaleDateString("pt-BR")}`,
      14,
      22
    );

    doc.text(`Faturamento: R$ ${totalSold.toFixed(2)}`, 14, 32);
    doc.text(`Lucro Extra (Trocos): R$ ${totalTrocoLucro.toFixed(2)}`, 14, 37);

    const tableData = sales.map((venda) => [
      venda.dataFormatada,
      venda.vendedor_nome,
      venda.pagamentoDisplay,
      venda.obsDisplay, // Adicionei OBS no PDF
      `R$ ${venda.total.toFixed(2)}`,
    ]);

    autoTable(doc, {
      head: [["Data", "Vendedor", "Pagto", "Obs/Troco", "Total"]],
      body: tableData,
      startY: 45,
      styles: { fontSize: 8 },
      columnStyles: { 3: { cellWidth: 40 } }, // Ajuste largura da coluna Obs
    });
  ...
  function exportExcel() {
    const excelData = sales.map((venda) => ({
      ...
      Observacao: venda.obsDisplay, // Adicionei OBS no Excel
      Total: venda.total,
    }));
        ...
        {sales.length > 0 && (
          <div className="animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              ...
              {/* NOVO CARD: LUCRO DE TROCO */}
              <div className="bg-white p-4 rounded shadow border-l-4 border-amber-500 flex justify-between">
                <div>
                  <p className="text-sm font-bold text-gray-500">
                    Lucro (Trocos)
                  </p>
                  <p className="text-xl font-bold text-amber-600">
                    R$ {totalTrocoLucro.toFixed(2)}
                  </p>
                </div>
                <Coins className="text-amber-200" size={28} />
              </div>

              <div className="bg-white p-4 rounded shadow border-l-4 border-blue-500 flex justify-between">
                <div>
                  <p className="text-sm font-bold text-gray-500">Qtd Vendas</p>
              ...
              <div className="bg-white p-4 rounded shadow border-l-4 border-orange-500 flex justify-between">
                ...
                <Package className="text-orange-200" size={28} />
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-6">
              <button
                onClick={exportPDF}
                className="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 flex items-center justify-center gap-2 shadow font-bold"
              >
                <FileText size={18} /> Baixar PDF
              </button>
              <button
                onClick={exportExcel}
                className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 flex items-center justify-center gap-2 shadow font-bold"
              >
            ...
            <table className="w-full text-left min-w-[800px]">
              <thead className="bg-gray-100 text-gray-700 uppercase text-xs font-bold">
                <tr>
                  ...
                  <th className="p-4">Obs / Troco</th> {/* Nova Coluna */}
              ...
              <tbody className="divide-y divide-gray-100">
                {sales.map((sale) => (
                  ...
                  >
                    ...
                    {/* COLUNA DE OBSERVAÇÃO / TROCO */}
                    <td className="p-4 text-xs">
                      {sale.isTrocoLucro ? (
                        <span className="text-amber-700 font-bold bg-amber-100 px-2 py-1 rounded border border-amber-200">
                          {sale.obsDisplay}
                        </span>
                      ) : (
